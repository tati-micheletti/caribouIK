...
#    `functionName` is a function's name in your module (e.g., `caribouIKEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Tati/Documents/GitHub/caribouIK/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("caribouIK")
  ...
  parameters <- list(
    ...
    caribouIK = list(.saveInitialTime = NA)
  )
  ...
  if (exists("caribouIKEvent1", envir = .GlobalEnv)) {
    simOutput <- caribouIKEvent1(mySim)
  } else {
  ...
  if (exists("caribouIKEvent2", envir = .GlobalEnv)) {
    simOutput <- caribouIKEvent2(mySim)
  } else {
